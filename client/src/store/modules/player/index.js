...
const triggerOauthIfNotLoggedIn = (opts) => {
  if (!Cookies.get(ACCESS_TOKEN_COOKIE_KEY)) {
    if (opts && opts.trackIndex) {
      window.open(`${API_HOST}/api/login?trackIndex=${opts.trackIndex}`, '_self')
    } else {
      window.open(`${API_HOST}/api/login`, '_self')
    }
    return false
  }
  return true
...
const actions = {
  ...
  [ACTION_TYPES.PLAY_TRACKS]({ state, rootState }, { index }) {
    if (triggerOauthIfNotLoggedIn({trackIndex: index}) && state.player && state.deviceId) {
      ...
      if (uris && uris.length > 0) {
        axios.put(`${API_HOST}/api/play`, { uris, deviceId: state.deviceId })
          .catch((error) => {
  ...
  [ACTION_TYPES.INIT_SPOTIFY_PLAYER]({ commit, state, dispatch }) {
    // Define the callback globally so it's available when SDK loads
    window.onSpotifyWebPlaybackSDKReady = () => {
      // Only initialize if user has access token
      if (!Cookies.get(ACCESS_TOKEN_COOKIE_KEY)) {
        return
      }
